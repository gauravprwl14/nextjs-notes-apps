...
const updateUserNoteValidator = z.object({
    data: z.object({
        note: z.any()
    })
})
...
const update = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    try {
        ...
        try {
            updateUserNoteValidator.parse(req.body)
        } catch (error) {
            const payload = {
                message: error
            }
        ...
        const notesRecord = await NotesModel.findOneAndUpdate(
            ...
            {
                $set: {
                    "personnelNote.note": note,
                    "personnelNote.plainText": plainText
                }
            },
            {
                new: true,
        ...
        Logger.error("update.user notes ", error)
